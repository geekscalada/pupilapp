...
CREATE TABLE public.streamdata (
	...
	insertdate timestamp(3) NOT NULL DEFAULT now(),
...
DROP ROLE IF exists metabaseuser;

CREATE ROLE metabaseuser NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT LOGIN PASSWORD 'pupil4!';
GRANT SELECT ON TABLE public.iddata TO metabaseuser;
GRANT SELECT ON TABLE public.matchdata TO metabaseuser;
GRANT SELECT ON TABLE public.oddsdata TO metabaseuser;
GRANT SELECT ON TABLE public.statusdata TO metabaseuser;
GRANT SELECT ON TABLE public.streamdata TO metabaseuser;
